...
- [x] **GATE-02**: Policy A (strict) — BUY: allow new entries up to 8 slots, target 100% exposure; CASH: hold existing, no new; SELL: liquidate all next open
- [x] **GATE-03**: Ceiling/floor lock handling — skip entry fill if `next_open == ceiling AND next_high == next_low`; defer exit if floor-locked, exit next open
- [x] **GATE-04**: T+2 settlement enforcement — bought day D not sellable until D+3

### Portfolio Engine

- [x] **PORT-01**: Multi-stock long-only state machine (max 8 concurrent positions)
- [x] **PORT-02**: Equal-weight position sizing (12.5%/slot), 100-share lot rounding (round down)
- [x] **PORT-03**: 8% hard stop loss
- [x] **PORT-04**: MA50 trailing stop (close break confirmed by volume ≥1.25× 20d avg)
- [x] **PORT-05**: Limit-down handling — if stop hit on limit-down day, exit next open
- [x] **PORT-06**: Exit priority chain — MDM SELL > hard stop > MA50 break > RS deterioration (RS<70 for 5 sessions)
- [x] **PORT-07**: Re-entry cooldown — 5 days per ticker after stop-out
- [x] **PORT-08**: Transaction costs — 0.25% commission both sides + 0.10% sell tax + 0.10% slippage
- [x] **PORT-09**: Liquidity gate — only enter if 20d ADV > 10× position size
...
- [x] **BT-05**: Performance report — CAGR, Sharpe (risk-free = 10Y VN govt ~3%), MaxDD, hit rate, profit factor, hold time, turnover, cost drag
- [x] **BT-06**: Benchmark comparison — VN-Index B&H, VN30 B&H, MDM-only-on-index, 12M deposit, SJC gold
- [x] **BT-07**: Real (inflation-adjusted) CAGR alongside nominal
